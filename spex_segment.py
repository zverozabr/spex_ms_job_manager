...
def nlm_denoise(image, patch, dist):
    ...
    def nlm_denoise_wrap(array):
        ...
        correct = denoise_nl_means(correct, h=0.6 * sigma_est, sigma=sigma_est,fast_mode=True,patch_size=patch,
                                   patch_distance=dist, multichannel=False, preserve_range=True)
        return correct[np.newaxis, ...]
    
    denoise = apply_parallel(nlm_denoise_wrap, image, chunks=(1, image.shape[1], image.shape[2]), dtype='float',
                             compute=True)
...
def median_denoise(image, kernal, ch):
    ...
    denoise = apply_parallel(median_denoise_wrap, image, chunks=(1, image.shape[1], image.shape[2]), dtype='float',
                             compute=True)
